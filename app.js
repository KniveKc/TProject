...
SerialPort.list(function(err, ports) { //Ports auslesen
    ports.forEach(function(port) {
        console.log(port.comName);
        ports1.push(port.comName);
    ...
    con.connect(function(err) {
        ...
        io.on("connection", function(socket) {
            ...
            //Zum auswählen des ComPorts

            socket.broadcast.emit('ports', ports1);
            ...
            socket.on("setZero", function() {
                var command = { command: "G10 P0 L20 X0 Y0 Z0" };
                serial.text(command.command);
                socket.emit("newCommand", command);
                socket.broadcast.emit("newCommand", command);
            });
